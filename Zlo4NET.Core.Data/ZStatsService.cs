...
internal class ZStatsService : IZStatsService
{
	...
	public async Task<ZStatsBase> GetStatsAsync(ZGame game)
	{
		...
		ZRequest request = ZRequestFactory.CreateStatsRequest(game);
		...
		else
		{
			...
			{
			case ZGame.BF3:
				stats = _parser.ParseBF3Stats(zResponse.ResponsePackets);
				break;
			case ZGame.BF4:
				stats = _parser.ParseBF4Stats(zResponse.ResponsePackets);
				break;
			}
		}
		return stats;
